...
import com.yedc.androidshared.system.copyToFile
import com.yedc.androidshared.system.getFileExtension
import com.yedc.androidshared.system.getFileName
import com.yedc.async.Scheduler
import com.yedc.maps.AnalyticsEvents
...
class OfflineMapLayersViewModel(
    ...
    fun loadLayersToImport(uris: List<Uri>, context: Context) {
        trackableWorker.immediate {
            ...
            uris.forEach { uri ->
                if (uri.getFileExtension(context) == _root_ide_package_.com.yedc.maps.layers.MbtilesFile.FILE_EXTENSION) {
                    uri.getFileName(context)?.let { fileName ->
                        ...
                        layers.add(
                            ReferenceLayer(
                                ...
                                layerFile,
                                _root_ide_package_.com.yedc.maps.layers.MbtilesFile.readName(layerFile) ?: layerFile.name
                            )
                        )
                    }
                }

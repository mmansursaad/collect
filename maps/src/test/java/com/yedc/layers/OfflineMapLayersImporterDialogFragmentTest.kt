...
import com.yedc.testshared.RecyclerViewMatcher.Companion.withRecyclerView
...
class OfflineMapLayersImporterDialogFragmentTest {
    ...
    fun `clicking the 'add layer' button dismisses the dialog`() {
        launchFragment().onFragment {
            ...
            Interactions.clickOn(withId(com.yedc.maps.R.id.add_layer_button))
            scheduler.flush()
    ...
    fun `progress indicator is displayed during loading layers`() {
        val file1 = TempFiles.createTempFile("layer1", _root_ide_package_.com.yedc.maps.layers.MbtilesFile.FILE_EXTENSION)
        val file2 = TempFiles.createTempFile("layer2", _root_ide_package_.com.yedc.maps.layers.MbtilesFile.FILE_EXTENSION)
        ...
        onView(withId(com.yedc.maps.R.id.progress_indicator)).check(matches(isDisplayed()))
        onView(withId(com.yedc.maps.R.id.layers)).check(matches(not(isDisplayed())))

        scheduler.flush()

        onView(withId(com.yedc.maps.R.id.progress_indicator)).check(matches(not(isDisplayed())))
        onView(withId(com.yedc.maps.R.id.layers)).check(matches(isDisplayed()))
    }
    ...
    fun `the 'cancel' button is enabled during loading layers`() {
        ...
        onView(withId(com.yedc.maps.R.id.cancel_button)).check(matches(isEnabled()))
        scheduler.flush()
        onView(withId(com.yedc.maps.R.id.cancel_button)).check(matches(isEnabled()))
    }
    ...
    fun `the 'add layer' button is disabled during loading layers`() {
        val file = TempFiles.createTempFile("layer", _root_ide_package_.com.yedc.maps.layers.MbtilesFile.FILE_EXTENSION)

        launchFragment().onFragment {
            it.viewModel.loadLayersToImport(listOf(file.toUri()), it.requireContext())
        }

        onView(withId(com.yedc.maps.R.id.add_layer_button)).check(matches(not(isEnabled())))
        scheduler.flush()
        onView(withId(com.yedc.maps.R.id.add_layer_button)).check(matches(isEnabled()))
    }
    ...
    fun `'All projects' location should be selected by default`() {
        ...
        onView(withId(com.yedc.maps.R.id.all_projects_option)).check(matches(isChecked()))
        onView(withId(com.yedc.maps.R.id.current_project_option)).check(matches(not(isChecked())))
    }
    ...
    fun `checking location sets selection correctly`() {
        ...
        Interactions.clickOn(withId(com.yedc.maps.R.id.current_project_option))

        onView(withId(com.yedc.maps.R.id.all_projects_option)).check(matches(not(isChecked())))
        onView(withId(com.yedc.maps.R.id.current_project_option)).check(matches(isChecked()))

        Interactions.clickOn(withId(com.yedc.maps.R.id.all_projects_option))
    ...
    fun `the list of selected layers should be displayed in A-Z order`() {
        val file1 = TempFiles.createTempFile("layerB", _root_ide_package_.com.yedc.maps.layers.MbtilesFile.FILE_EXTENSION)
        val file2 = TempFiles.createTempFile("layerA", _root_ide_package_.com.yedc.maps.layers.MbtilesFile.FILE_EXTENSION)
        ...
        onView(withId(com.yedc.maps.R.id.layers)).check(
            matches(
        ...
        onView(
            withRecyclerView(com.yedc.maps.R.id.layers).atPositionOnView(
                0,
                com.yedc.maps.R.id.layer_name
            )
    ...
    fun `the warning dialog is displayed if some selected files are not supported and the importer dialog is kept displayed`() {
        ...
        val file2 = TempFiles.createTempFile("layerB", _root_ide_package_.com.yedc.maps.layers.MbtilesFile.FILE_EXTENSION)
    ...
    fun `the warning dialog shows correct number of unsupported layers`() {
        ...
        launchFragment().onFragment {
            ...
            it.viewModel.loadLayersToImport(
                listOf(
                    TempFiles.createTempFile("layerA", ".txt").toUri(),
                    TempFiles.createTempFile("layerB", ".txt").toUri(),
                    TempFiles.createTempFile("layerC", _root_ide_package_.com.yedc.maps.layers.MbtilesFile.FILE_EXTENSION).toUri()
                ),
                context
            )
            scheduler.flush()
        ...
        launchFragment().onFragment {
            ...
            it.viewModel.loadLayersToImport(
                listOf(
                    TempFiles.createTempFile("layerA", ".txt").toUri(),
                    TempFiles.createTempFile("layerB", _root_ide_package_.com.yedc.maps.layers.MbtilesFile.FILE_EXTENSION).toUri(),
                    TempFiles.createTempFile("layerC", _root_ide_package_.com.yedc.maps.layers.MbtilesFile.FILE_EXTENSION).toUri()
                ),
                context
            )

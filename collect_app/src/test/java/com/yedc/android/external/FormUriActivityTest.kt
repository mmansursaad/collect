...
import com.yedc.formstest.InMemSavepointsRepository
...
class FormUriActivityTest {
    ...
    private val instancesRepository = com.yedc.formstest.InMemInstancesRepository { 0 }
    ...
    fun `When there are no projects then display alert dialog`() {
        ...
        assertErrorDialogAndClickCancelButton(
            scenario,
            context.getString(com.yedc.strings.R.string.app_not_configured)
        )
    }
    ...
    fun `When there is project id specified in uri but it is not the currently used project then display alert dialog`() {
        ...
        assertErrorDialogAndClickCancelButton(
            scenario,
            context.getString(com.yedc.strings.R.string.wrong_project_selected_for_form)
        )
    }
    ...
    fun `When uri is null then display alert dialog`() {
        ...
        assertErrorDialogAndClickCancelButton(
            scenario,
            context.getString(com.yedc.strings.R.string.unrecognized_uri)
        )
    }
    ...
    fun `When uri represents a blank form that does not exist then display alert dialog`() {
        ...
        assertErrorDialogAndClickCancelButton(scenario, context.getString(com.yedc.strings.R.string.bad_uri))
    }
    ...
    fun `When attempting to edit a form with non existing instance file then display alert dialog and remove the instance from the database`() {
        ...
        assertErrorDialogAndClickCancelButton(
            scenario,
            context.getString(com.yedc.strings.R.string.instance_deleted_message)
        )
    }
    ...
    fun `When attempting to edit a form with zero form definitions then display alert dialog with formId if version does not exist`() {
        ...
        val expectedMessage = context.getString(
            com.yedc.strings.R.string.parent_form_not_present,
            instance.formId
        )
    ...
    fun `When attempting to edit a form with zero form definitions then display alert dialog with formId and version if both exist`() {
        ...
        val expectedMessage = context.getString(
            com.yedc.strings.R.string.parent_form_not_present,
            "${instance.formId}\n${
                context.getString(com.yedc.strings.R.string.version)
    ...
    fun `When attempting to edit a form with multiple form definitions then display alert dialog`() {
        ...
        assertErrorDialogAndClickCancelButton(
            scenario,
            context.getString(com.yedc.strings.R.string.survey_multiple_forms_error)
        )
    }
    ...
    fun `When attempting to edit an encrypted form then display alert dialog`() {
        ...
        assertErrorDialogAndClickCancelButton(
            scenario,
            context.getString(com.yedc.strings.R.string.encrypted_form)
        )
    }
    ...
    fun `Form filling should not be started again after recreating the activity`() {
        ...
        Intents.intended(hasComponent(_root_ide_package_.com.yedc.android.activities.FormFillingActivity::class.java.name), Intents.times(1))
    }
    ...
    fun `When there is project id specified in uri that represents an invalid form and it matches current project id then start form filling`() {
        ...
        val instance = instancesRepository.save(
            com.yedc.forms.instances.Instance.Builder()
                ...
                .formVersion("1")
                .instanceFilePath(TempFiles.createTempFile(TempFiles.createTempDir()).absolutePath)
                .status(com.yedc.forms.instances.Instance.STATUS_INVALID)
                .build()
        )
    ...
    fun `If there is a savepoint for older version of the blank form, display a recovery dialog and start the old version of the blank form if a user accepts`() {
        ...
        assertNonCancelableSavepointRecoveryDialog(savepointFile)
        onView(withText(com.yedc.strings.R.string.recover)).perform(click())
    ...
    fun `If there is a savepoint for older version of the blank form, display a recovery dialog and start the new version of the blank form if a user declines`() {
        ...
        assertNonCancelableSavepointRecoveryDialog(savepointFile)
        onView(withText(com.yedc.strings.R.string.do_not_recover)).perform(click())
        fakeScheduler.flush()
    ...
    fun `When attempting to start a new form that uses entities and the forms database is locked then display alert dialog`() {
        ...
        assertErrorDialogAndClickCancelButton(
            scenario,
            context.getString(com.yedc.strings.R.string.cannot_open_form_because_of_forms_update)
        )
    }
    ...
    private fun getBlankFormIntent(projectId: String?, dbId: Long) =
        Intent(context, FormUriActivity::class.java).apply {
            data = if (projectId == null) {
                ...
                _root_ide_package_.com.yedc.android.external.FormsContract.getUri(projectId, dbId)
            }
            putExtra("KEY_1", "Text")
        }
    ...
    private fun getSavedIntent(projectId: String?, dbId: Long) =
        Intent(context, FormUriActivity::class.java).apply {
            data = if (projectId == null) {
                ...
                _root_ide_package_.com.yedc.android.external.InstancesContract.getUri(projectId, dbId)
            }
            putExtra("KEY_1", "Text")
        }

    private fun getFormsUriInOldFormatWithNoProjectId(dbId: Long): Uri {
        val uri = _root_ide_package_.com.yedc.android.external.FormsContract.getUri("", dbId)
        return Uri.Builder()
            .scheme(uri.scheme)
    ...
    private fun getInstancesUriInOldFormatWithNoProjectId(dbId: Long): Uri {
        val uri = _root_ide_package_.com.yedc.android.external.InstancesContract.getUri("", dbId)
        return Uri.Builder()
            .scheme(uri.scheme)
            .authority(uri.authority)
            .path(uri.path)
    ...
    private fun assertErrorDialogAndClickCancelButton(scenario: ActivityScenario<FormUriActivity>, message: String) {
        onView(withText(com.yedc.strings.R.string.form_cannot_be_used)).inRoot(isDialog()).check(matches(isDisplayed()))
    ...
    private fun assertNonCancelableSavepointRecoveryDialog(savepointFile: File) {
        onView(isRoot()).perform(pressBack())
        onView(withText(com.yedc.strings.R.string.savepoint_recovery_dialog_title)).inRoot(isDialog()).check(matches(isDisplayed()))
        onView(withText(SimpleDateFormat(context.getString(com.yedc.strings.R.string.savepoint_recovery_dialog_message), Locale.getDefault()).format(savepointFile.lastModified()))).inRoot(isDialog()).check(matches(isDisplayed()))
        onView(withText(com.yedc.strings.R.string.recover)).inRoot(isDialog()).check(matches(isDisplayed()))
        onView(withText(com.yedc.strings.R.string.do_not_recover)).inRoot(isDialog()).check(matches(isDisplayed()))
    }

    private fun assertStartBlankFormIntent(projectId: String?, dbId: Long) {
        Intents.intended(hasComponent(_root_ide_package_.com.yedc.android.activities.FormFillingActivity::class.java.name))
        if (projectId == null) {
            Intents.intended(hasData(getFormsUriInOldFormatWithNoProjectId(dbId)))
        } else {
            Intents.intended(hasData(_root_ide_package_.com.yedc.android.external.FormsContract.getUri(projectId, dbId)))
        }
    ...
    private fun assertStartSavedFormIntent(projectId: String?, dbId: Long, canBeEdited: Boolean) {
        ...
        if (projectId == null) {
            ...
            Intents.intended(hasData(_root_ide_package_.com.yedc.android.external.InstancesContract.getUri(projectId, dbId)))
        }
        if (canBeEdited) {
            Intents.intended(not(hasExtraWithKey(FormOpeningMode.FORM_MODE_KEY)))
        } else {

...
import com.yedc.android.widgets.CounterWidget.Companion.MAX_VALUE

class CounterWidgetTest : com.yedc.android.widgets.base.QuestionWidgetTest<CounterWidget, IAnswerData>() {

    override fun createWidget() = CounterWidget(
        activity,
        _root_ide_package_.com.yedc.android.formentry.questions.QuestionDetails(formEntryPrompt),
        dependencies
    )
    ...
    fun `clicking the minus button calls widgetValueChanged listener`() {
        whenever(formEntryPrompt.answerValue).thenReturn(IntegerData(1))
        val valueChangedListener = com.yedc.android.widgets.support.QuestionWidgetHelpers.mockValueChangedListener<_root_ide_package_.com.yedc.android.widgets.QuestionWidget>(widget)

...
import com.yedc.testshared.getOrAwaitValue
...
class BlankFormListViewModelTest {
    ...
    fun `isMatchExactlyEnabled returns correct value based on settings`() {
        generalSettings.save(
            ...
            com.yedc.settings.enums.FormUpdateMode.MANUAL.getValue(context)
        )
        ...
        generalSettings.save(
            ProjectKeys.KEY_FORM_UPDATE_MODE,
            com.yedc.settings.enums.FormUpdateMode.MATCH_EXACTLY.getValue(context)
        )
    ...
    private fun saveForms(vararg forms: com.yedc.forms.Form) {
        whenever(formsDataService.getForms(any())).thenReturn(MutableStateFlow(forms.toList()))
    }

    private fun saveInstances(vararg instances: com.yedc.forms.instances.Instance) {
    ...
    private fun assertFormItem(blankFormListItem: BlankFormListItem, form: com.yedc.forms.Form, lastStatusChangeDate: Long = 0) {
        assertThat(
            blankFormListItem,
            `is`(

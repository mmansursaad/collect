...
import com.yedc.android.formentry.FormOpeningMode.FORM_MODE_KEY
import com.yedc.android.formentry.FormOpeningMode.VIEW_SENT
...
class FormFillingIntentFactoryTest {
    ...
    fun `newInstance starts FormUriActivity with instance URI`() {
        activity.startActivity(FormFillingIntentFactory.newFormIntent(activity, _root_ide_package_.com.yedc.android.external.InstancesContract.getUri("projectId", 101)))

        intended(hasAction(ACTION_EDIT))
        intended(hasComponent(FormUriActivity::class.java.name))
        intended(hasData(_root_ide_package_.com.yedc.android.external.InstancesContract.getUri("projectId", 101)))

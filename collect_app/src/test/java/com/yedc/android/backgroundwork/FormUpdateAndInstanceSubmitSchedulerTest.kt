...
import com.yedc.settings.enums.FormUpdateMode.MATCH_EXACTLY
import com.yedc.settings.enums.FormUpdateMode.PREVIOUSLY_DOWNLOADED_ONLY
import com.yedc.settings.keys.ProjectKeys
import com.yedc.settings.keys.ProjectKeys.KEY_FORM_UPDATE_MODE
import com.yedc.settings.keys.ProjectKeys.KEY_PERIODIC_FORM_UPDATES_CHECK
...
class FormUpdateAndInstanceSubmitSchedulerTest {
    ...
    fun `scheduleUpdates passes project id when scheduling previously downloaded only`() {
        ...
        generalSettings.save(
            ...
            application.getString(com.yedc.strings.R.string.every_one_hour_value)
        )

        val manager =
            _root_ide_package_.com.yedc.android.backgroundwork.FormUpdateAndInstanceSubmitScheduler(
                scheduler,
                settingsProvider,
                application
            )

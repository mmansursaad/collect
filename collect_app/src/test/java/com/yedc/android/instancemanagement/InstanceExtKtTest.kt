...
class InstanceExtKtTest {
    ...
    fun getStatusDescriptionTest() {
        val incomplete = InstanceFixtures.instance(status = com.yedc.forms.instances.Instance.STATUS_INCOMPLETE)
        assertDateFormat(
            incomplete.getStatusDescription(resources),
            R.string.saved_on_date_at_time
        )

        val invalid = InstanceFixtures.instance(status = com.yedc.forms.instances.Instance.STATUS_INVALID)
        assertDateFormat(
            invalid.getStatusDescription(resources),
            R.string.saved_on_date_at_time
        )

        val valid = InstanceFixtures.instance(status = com.yedc.forms.instances.Instance.STATUS_VALID)
        assertDateFormat(
            valid.getStatusDescription(resources),
            R.string.saved_on_date_at_time
        )

        val newEdit = InstanceFixtures.instance(status = com.yedc.forms.instances.Instance.STATUS_NEW_EDIT)
        assertDateFormat(
            newEdit.getStatusDescription(resources),
            R.string.saved_on_date_at_time
        )

        val complete = InstanceFixtures.instance(status = com.yedc.forms.instances.Instance.STATUS_COMPLETE)
        assertDateFormat(
            complete.getStatusDescription(resources),
            R.string.finalized_on_date_at_time
        )

        val submitted = InstanceFixtures.instance(status = com.yedc.forms.instances.Instance.STATUS_SUBMITTED)
        assertDateFormat(
            submitted.getStatusDescription(resources),
            R.string.sent_on_date_at_time
        )

        val submissionFailed = InstanceFixtures.instance(status = com.yedc.forms.instances.Instance.STATUS_SUBMISSION_FAILED)
        assertDateFormat(
    ...
    fun `isDeletable returns true if canDeleteBeforeSend is true no matter what the status is`() {
        listOf(
            ...
            val instance = com.yedc.forms.instances.Instance
                .Builder()
                .status(status)

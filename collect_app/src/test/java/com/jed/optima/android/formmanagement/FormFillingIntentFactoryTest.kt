...
import com.jed.optima.android.formentry.FormOpeningMode.FORM_MODE_KEY
import com.jed.optima.android.formentry.FormOpeningMode.VIEW_SENT
...
class FormFillingIntentFactoryTest {
    ...
    fun `newInstance starts FormUriActivity with instance URI`() {
        activity.startActivity(FormFillingIntentFactory.newFormIntent(activity, com.jed.optima.android.external.InstancesContract.getUri("projectId", 101)))

        intended(hasAction(ACTION_EDIT))
        intended(hasComponent(FormUriActivity::class.java.name))
        intended(hasData(com.jed.optima.android.external.InstancesContract.getUri("projectId", 101)))

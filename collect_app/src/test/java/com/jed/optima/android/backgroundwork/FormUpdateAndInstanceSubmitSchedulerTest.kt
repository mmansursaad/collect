...
import com.jed.optima.settings.enums.FormUpdateMode.MATCH_EXACTLY
import com.jed.optima.settings.enums.FormUpdateMode.PREVIOUSLY_DOWNLOADED_ONLY
import com.jed.optima.settings.keys.ProjectKeys
import com.jed.optima.settings.keys.ProjectKeys.KEY_FORM_UPDATE_MODE
import com.jed.optima.settings.keys.ProjectKeys.KEY_PERIODIC_FORM_UPDATES_CHECK
...
class FormUpdateAndInstanceSubmitSchedulerTest {
    ...
    fun `scheduleUpdates passes project id when scheduling previously downloaded only`() {
        ...
        generalSettings.save(
            ...
            application.getString(com.jed.optima.strings.R.string.every_one_hour_value)
        )

        val manager = com.jed.optima.android.backgroundwork.FormUpdateAndInstanceSubmitScheduler(
            scheduler,
            settingsProvider,
            application
        )

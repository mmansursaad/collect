...
import com.jed.optima.testshared.getOrAwaitValue
...
class BlankFormListViewModelTest {
    ...
    fun `isMatchExactlyEnabled returns correct value based on settings`() {
        generalSettings.save(
            ...
            com.jed.optima.settings.enums.FormUpdateMode.MANUAL.getValue(context)
        )
        ...
        generalSettings.save(
            ProjectKeys.KEY_FORM_UPDATE_MODE,
            com.jed.optima.settings.enums.FormUpdateMode.MATCH_EXACTLY.getValue(context)
        )
    ...
    private fun saveForms(vararg forms: com.jed.optima.forms.Form) {
        whenever(formsDataService.getForms(any())).thenReturn(MutableStateFlow(forms.toList()))
    }

    private fun saveInstances(vararg instances: com.jed.optima.forms.instances.Instance) {
    ...
    private fun assertFormItem(blankFormListItem: BlankFormListItem, form: com.jed.optima.forms.Form, lastStatusChangeDate: Long = 0) {
        assertThat(
            blankFormListItem,
            `is`(

...
class FormMapViewModelTest {
    ...
    fun `returns items based on form instances with point geometry`() {
        ...
        val expectedItem = MappableSelectItem.MappableSelectPoint(
            ...
            action = IconifiedText(
                R.drawable.ic_edit,
                application.getString(com.jed.optima.strings.R.string.edit_data)
            ),
            info = formatDate(
                com.jed.optima.strings.R.string.saved_on_date_at_time,
    ...
    fun `finalized instances with geometry have view action and no info`() {
        ...
        val expectedItem = MappableSelectItem.MappableSelectPoint(
            ...
            action = IconifiedText(
                ...
                application.getString(com.jed.optima.strings.R.string.view_data)
            ),
            info = formatDate(
                com.jed.optima.strings.R.string.finalized_on_date_at_time,
                instance.lastStatusChangeDate
            )
        )
        assertThat(viewModel.getMappableItems().value!![0], equalTo(expectedItem))
    ...
    fun `deleted instances with geometry have info and no action`() {
        ...
        val expectedItem = MappableSelectItem.MappableSelectPoint(
            ...
            info = formatDate(
                ...
                formatDate(
                    com.jed.optima.strings.R.string.deleted_on_date_at_time,
                    123L
                )
        )
        assertThat(viewModel.getMappableItems().value!![0], equalTo(expectedItem))
    ...
    fun `submitted instances with geometry have view action and no info`() {
        ...
        val expectedItem = MappableSelectItem.MappableSelectPoint(
            ...
            info = formatDate(
                com.jed.optima.strings.R.string.sent_on_date_at_time,
                instance.lastStatusChangeDate
            )
        )
        assertThat(viewModel.getMappableItems().value!![0], equalTo(expectedItem))
    ...
    fun `instances that failed to submit with geometry have view action and no info`() {
        ...
        val expectedItem = MappableSelectItem.MappableSelectPoint(
            ...
            info = formatDate(
                com.jed.optima.strings.R.string.sending_failed_on_date_at_time,
                instance.lastStatusChangeDate
            )
        )
        assertThat(viewModel.getMappableItems().value!![0], equalTo(expectedItem))
    ...
    fun `instances that cannot be edited after completion with geometry have info and no action`() {
        ...
        assertThat(
            items!![0].info,
            containsString(application.getString(com.jed.optima.strings.R.string.cannot_edit_completed_form))
        )
    ...
    private fun createAndLoadViewModel(form: com.jed.optima.forms.Form): FormMapViewModel {
        val viewModel = createViewModel(form)
        viewModel.load()
        scheduler.flush()
        return viewModel
    ...
    private fun createViewModel(form: com.jed.optima.forms.Form): FormMapViewModel {

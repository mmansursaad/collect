...
class HtmlUtilsTest {
    ...
    fun `#textToHtml null becomes empty string`() {
        val observed = com.jed.optima.android.utilities.HtmlUtils.textToHtml(null)
        assertThat(observed, equalTo(""))
    }

    @Test
    fun `#textToHtml should be trimmed`() {
        val observed = com.jed.optima.android.utilities.HtmlUtils.textToHtml("<p style=\"text-align:center\">Text</p>")
        assertThat(observed.toString(), equalTo("Text"))
    }

    @Test
    fun `#textToHtml ignores invalid styles`() {
        var observed = com.jed.optima.android.utilities.HtmlUtils.textToHtml("<p style=>Text</p>")
        assertThat(observed.toString(), equalTo("Text"))

        observed = com.jed.optima.android.utilities.HtmlUtils.textToHtml("<span style=\"color:\">Text</span>")
        assertThat(observed.toString(), equalTo("Text"))

        observed = com.jed.optima.android.utilities.HtmlUtils.textToHtml("<span style=\"font-family\">Text</span>")
        assertThat(observed.toString(), equalTo("Text"))
    }
    ...
    fun `#markDownToHtml escapes backslash`() {
        ...
        for (testCase in tests) {
            assertThat(testCase[1], equalTo(com.jed.optima.android.utilities.HtmlUtils.markdownToHtml(testCase[0])))
        }
    }
    ...
    fun `#markDownToHtml supports HTML`() {
        ...
        for (testCase in tests) {
            assertThat(testCase, equalTo(com.jed.optima.android.utilities.HtmlUtils.markdownToHtml(testCase)))
        }
    }
    ...
    fun `#textToHtml supports escaped lt`() {
        ...
        for (testCase in tests) {
            assertThat(testCase, equalTo(com.jed.optima.android.utilities.HtmlUtils.textToHtml(testCase).toString()))
        }
    }
}

...
class BlankFormListMenuProviderTest {
    ...
    fun setup() {
        ...
        activity = com.jed.optima.android.support.CollectHelpers.createThemedActivity(FragmentActivity::class.java)
    }
    ...
    fun `clicking refresh for sync shows success toast`() {
        ...
        assertThat(
            ...
            `is`(activity.getString(com.jed.optima.strings.R.string.form_update_succeeded))
        )
    }
    ...
    fun `clicking refresh for sync when device is offline shows error toast and does not sync`() {
        ...
        assertThat(
            ShadowToast.getTextOfLatestToast(),
            `is`(activity.getString(com.jed.optima.strings.R.string.no_connection))
        )

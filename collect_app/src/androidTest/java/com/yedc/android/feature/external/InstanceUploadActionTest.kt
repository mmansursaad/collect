...
class InstanceUploadActionTest {
    ...
    fun whenIntentIncludesURLExtra_instancesAreUploadedToThatURL() {
        ...
        val instanceId =
            context.contentResolver.query(_root_ide_package_.com.yedc.android.external.InstancesContract.getUri("DEMO"), null, null, null, null)
                .use {
                    it!!.moveToFirst()
                    it.getLong(it.getColumnIndex(_ID))
                }

        val intent = Intent("com.yedc.android.INSTANCE_UPLOAD")
        intent.type = _root_ide_package_.com.yedc.android.external.InstancesContract.CONTENT_TYPE
        intent.putExtra(_root_ide_package_.com.yedc.android.utilities.ApplicationConstants.BundleKeys.URL, testDependencies.server.url)
        intent.putExtra("instances", longArrayOf(instanceId))

        rule.launch(intent, com.yedc.android.support.pages.OkDialog())
    ...
    fun whenInstanceDoesNotExist_showsError() {
        ...
        rule.launch(intent, com.yedc.android.support.pages.OkDialog())
            .assertText(com.yedc.strings.R.string.no_forms_uploaded)
    }
}

...
import com.yedc.android.formmanagement.metadata.FormMetadataParser.readMetadata
...
class StubOpenRosaServer : com.yedc.openrosa.http.OpenRosaHttpInterface {
    ...
    override fun executeGetRequest(
        uri: URI,
        contentType: String?,
        credentials: com.yedc.openrosa.http.HttpCredentialsInterface
    ): com.yedc.openrosa.http.HttpGetResult {
        accesses += 1

        if (alwaysReturnError) {
            return com.yedc.openrosa.http.HttpGetResult(null, HashMap(), "", 500)
        }

        if (uri.host != hostName) {
            return com.yedc.openrosa.http.HttpGetResult(
                null,
                HashMap(),
            ...
            return com.yedc.openrosa.http.HttpGetResult(null, HashMap(), "", 401)
            ...
            return if (manifestResponse != null) {
                ...
                com.yedc.openrosa.http.HttpGetResult(null, HashMap(), "", 404)
            }
        } else if (uri.path.startsWith("/mediaFile")) {
            return com.yedc.openrosa.http.HttpGetResult(getMediaFile(uri), HashMap(), "", 200)
            ...
            return com.yedc.openrosa.http.HttpGetResult(null, HashMap(), "", 404)
        }
    }
    ...
    override fun executeHeadRequest(
        uri: URI,
        credentials: com.yedc.openrosa.http.HttpCredentialsInterface
    ): com.yedc.openrosa.http.HttpHeadResult {
        accesses += 1

        if (alwaysReturnError) {
            return com.yedc.openrosa.http.HttpHeadResult(
                500,
                com.yedc.openrosa.http.CaseInsensitiveEmptyHeaders()
            )
        }
        ...
        if (uri.host != hostName) {
            ...
            return com.yedc.openrosa.http.HttpHeadResult(204, MapHeaders(headers))
        } else {
            return com.yedc.openrosa.http.HttpHeadResult(
                404,
                com.yedc.openrosa.http.CaseInsensitiveEmptyHeaders()
    ...
    override fun uploadSubmissionAndFiles(
        ...
        credentials: com.yedc.openrosa.http.HttpCredentialsInterface,
        contentLength: Long
    ): com.yedc.openrosa.http.HttpPostResult {
        accesses += 1

        if (alwaysReturnError) {
            return com.yedc.openrosa.http.HttpPostResult("", 500, "")
        }

        if (uri.host != hostName) {
            return com.yedc.openrosa.http.HttpPostResult(
                "Trying to connect to incorrect server: " + uri.host,
                410,
                ""
            )
        } else if (credentialsIncorrect(credentials)) {
            return com.yedc.openrosa.http.HttpPostResult("", 401, "")
        } else if (uri.path == OpenRosaConstants.SUBMISSION) {
            submittedForms.add(submissionFile)
            return com.yedc.openrosa.http.HttpPostResult("", 201, "")
        } else {
            return com.yedc.openrosa.http.HttpPostResult("", 404, "")
        }
    }
    ...
    fun addForm(formXML: String, mediaFiles: List<MediaFileItem> = emptyList()) {
        try {
            _root_ide_package_.com.yedc.android.utilities.FileUtils.getResourceAsStream("forms/$formXML").use { formDefStream ->
    ...
    private fun credentialsIncorrect(credentials: com.yedc.openrosa.http.HttpCredentialsInterface?): Boolean {
        ...
        if (xformItem.mediaFiles.isEmpty()) {
            ...
            for (mediaFile in xformItem.mediaFiles) {
                val mediaFileHash = mediaFile.hash
                    ?: if (randomHash) {
                        com.yedc.shared.strings.RandomString.randomString(8)
                    } else {
                        getMd5Hash(_root_ide_package_.com.yedc.android.utilities.FileUtils.getResourceAsStream("media/" + mediaFile.file)!!)
                    }
    ...
    private fun getFormXML(formID: String): InputStream {
        ...
        return _root_ide_package_.com.yedc.android.utilities.FileUtils.getResourceAsStream("forms/$xmlPath")!!
    }
    ...
    private fun getMediaFile(uri: URI): InputStream {
        ...
        val actualFileName =
            xformItem.mediaFiles.stream().filter { mediaFile: MediaFileItem -> mediaFile.id == id }
                .findFirst().get().file
        return _root_ide_package_.com.yedc.android.utilities.FileUtils.getResourceAsStream("media/$actualFileName")!!
    }
    ...
    private class MapHeaders(private val headers: Map<String, String>) :
        com.yedc.openrosa.http.CaseInsensitiveHeaders {

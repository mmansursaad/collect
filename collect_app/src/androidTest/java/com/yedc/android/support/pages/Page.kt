...
import com.yedc.android.support.rules.RecentAppsRule
import com.yedc.android.utilities.ActionRegister
import com.yedc.androidshared.ui.ToastUtils.popRecordedToasts
...
abstract class Page<T : Page<T>> {
    ...
    fun getTranslatedString(id: Int?, vararg formatArgs: Any): String {
        return ApplicationProvider.getApplicationContext<_root_ide_package_.com.yedc.android.application.Collect>()
    ...
    protected fun assertToolbarTitle(title: String?) {
        onView(
            allOf(
                withText(title),
                isDescendantOfA(withId(com.yedc.androidshared.R.id.toolbar))
            )
    ...
    fun closeSnackbar(): T {
        Interactions.clickOn(withContentDescription(com.yedc.strings.R.string.close_snackbar))
        return this as T
    }
    ...
    fun <D : Page<D>> killAndReopenApp(
        ...
        destination: D,
        appDependencyModule: _root_ide_package_.com.yedc.android.injection.config.AppDependencyModule? = null
    ): D {
    ...
    companion object {
        private fun rotateToLandscape(): ViewAction {
            return com.yedc.android.support.actions.RotateAction(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE)
        }

        private fun rotateToPortrait(): ViewAction {
            return com.yedc.android.support.actions.RotateAction(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT)
        }

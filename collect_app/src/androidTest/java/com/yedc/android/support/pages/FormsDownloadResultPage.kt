...
class FormsDownloadResultPage : Page<FormsDownloadResultPage>() {
    ...
    override fun assertOnPage(): FormsDownloadResultPage {
        onView(withText(com.yedc.strings.R.string.ok)).inRoot(RootMatchers.isDialog()).check(matches(isDisplayed()))
        return this
    }
    ...
    fun assertSuccess(): FormsDownloadResultPage {
        assertText(com.yedc.strings.R.string.all_downloads_succeeded)
        return this
    }

    fun showDetails(): ErrorPage {
        onView(withText(getTranslatedString(com.yedc.strings.R.string.show_details))).perform(click())

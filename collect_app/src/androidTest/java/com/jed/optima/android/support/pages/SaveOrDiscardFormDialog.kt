...
class SaveOrDiscardFormDialog<D : Page<D>> @JvmOverloads constructor(
    ...
    override fun assertOnPage(): SaveOrDiscardFormDialog<D> {
        if (saveAsDraftEnabled) {
            assertTextInDialog(com.jed.optima.strings.R.string.quit_form_title)
        } else {
            assertTextInDialog(com.jed.optima.strings.R.string.quit_form_continue_title)
        }
    ...
    fun clickSaveChanges(): D {
        return clickOnTextInDialog(com.jed.optima.strings.R.string.save_as_draft, destination)
    }

    fun clickSaveChangesWithError(errorMsg: Int): D {
        clickOnTextInDialog(com.jed.optima.strings.R.string.save_as_draft)
    ...
    fun clickDiscardForm(): D {
        return clickOnTextInDialog(com.jed.optima.strings.R.string.do_not_save, destination)
    }

...
import com.jed.optima.android.formmanagement.metadata.FormMetadataParser.readMetadata
import com.jed.optima.openrosa.http.OpenRosaConstants
...
class StubOpenRosaServer : com.jed.optima.openrosa.http.OpenRosaHttpInterface {
    ...
    override fun executeGetRequest(
        uri: URI,
        contentType: String?,
        credentials: com.jed.optima.openrosa.http.HttpCredentialsInterface
    ): com.jed.optima.openrosa.http.HttpGetResult {
        accesses += 1

        if (alwaysReturnError) {
            return com.jed.optima.openrosa.http.HttpGetResult(null, HashMap(), "", 500)
        }

        if (uri.host != hostName) {
            return com.jed.optima.openrosa.http.HttpGetResult(
                null,
                HashMap(),
            ...
            return com.jed.optima.openrosa.http.HttpGetResult(null, HashMap(), "", 401)
            ...
            return if (manifestResponse != null) {
                ...
                com.jed.optima.openrosa.http.HttpGetResult(null, HashMap(), "", 404)
            }
        } else if (uri.path.startsWith("/mediaFile")) {
            return com.jed.optima.openrosa.http.HttpGetResult(getMediaFile(uri), HashMap(), "", 200)
        } else if (uri.path.startsWith("/integrityUrl")) {
            return com.jed.optima.openrosa.http.HttpGetResult(
                getIntegrityResponse(uri),
                standardHeaders,
                "",
                200
            )
        } else {
            return com.jed.optima.openrosa.http.HttpGetResult(null, HashMap(), "", 404)
        }
    }
    ...
    override fun executeHeadRequest(
        uri: URI,
        credentials: com.jed.optima.openrosa.http.HttpCredentialsInterface
    ): com.jed.optima.openrosa.http.HttpHeadResult {
        accesses += 1

        if (alwaysReturnError) {
            return com.jed.optima.openrosa.http.HttpHeadResult(
                500,
                com.jed.optima.openrosa.http.CaseInsensitiveEmptyHeaders()
            )
        }
        ...
        if (uri.host != hostName) {
            ...
            return com.jed.optima.openrosa.http.HttpHeadResult(204, MapHeaders(headers))
        } else {
            return com.jed.optima.openrosa.http.HttpHeadResult(
                404,
                com.jed.optima.openrosa.http.CaseInsensitiveEmptyHeaders()
    ...
    override fun uploadSubmissionAndFiles(
        ...
        credentials: com.jed.optima.openrosa.http.HttpCredentialsInterface,
        contentLength: Long
    ): com.jed.optima.openrosa.http.HttpPostResult {
        accesses += 1

        if (alwaysReturnError) {
            return com.jed.optima.openrosa.http.HttpPostResult("", 500, "")
        }

        if (uri.host != hostName) {
            return com.jed.optima.openrosa.http.HttpPostResult(
                "Trying to connect to incorrect server: " + uri.host,
                410,
                ""
            )
        } else if (credentialsIncorrect(credentials)) {
            return com.jed.optima.openrosa.http.HttpPostResult("", 401, "")
        } else if (uri.path == OpenRosaConstants.SUBMISSION) {
            submittedForms.add(submissionFile)
            return com.jed.optima.openrosa.http.HttpPostResult("", 201, "")
        } else {
            return com.jed.optima.openrosa.http.HttpPostResult("", 404, "")
        }
    }
    ...
    fun addForm(formXML: String, mediaFiles: List<MediaFileItem> = emptyList()) {
        try {
            com.jed.optima.android.utilities.FileUtils.getResourceAsStream("forms/$formXML").use { formDefStream ->
    ...
    private fun credentialsIncorrect(credentials: com.jed.optima.openrosa.http.HttpCredentialsInterface?): Boolean {
        ...
        if (xformItem.mediaFiles.isEmpty()) {
            ...
            for (mediaFile in xformItem.mediaFiles) {
                val mediaFileHash = mediaFile.hash
                    ?: if (randomHash) {
                        com.jed.optima.shared.strings.RandomString.randomString(8)
                    } else {
                        getMd5Hash(com.jed.optima.android.utilities.FileUtils.getResourceAsStream("media/" + mediaFile.file)!!)
                    }
    ...
    private fun getFormXML(formID: String): InputStream {
        ...
        return com.jed.optima.android.utilities.FileUtils.getResourceAsStream("forms/$xmlPath")!!
    }
    ...
    private fun getMediaFile(uri: URI): InputStream {
        ...
        val actualFileName =
            xformItem.mediaFiles.stream().filter { mediaFile: MediaFileItem -> mediaFile.id == id }
                .findFirst().get().file
        return com.jed.optima.android.utilities.FileUtils.getResourceAsStream("media/$actualFileName")!!
    }
    ...
    private class MapHeaders(private val headers: Map<String, String>) :
        com.jed.optima.openrosa.http.CaseInsensitiveHeaders {

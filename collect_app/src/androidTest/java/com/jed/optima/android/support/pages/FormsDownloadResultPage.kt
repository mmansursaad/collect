...
class FormsDownloadResultPage : Page<FormsDownloadResultPage>() {
    ...
    override fun assertOnPage(): FormsDownloadResultPage {
        onView(withText(com.jed.optima.strings.R.string.ok)).inRoot(RootMatchers.isDialog()).check(matches(isDisplayed()))
        return this
    }
    ...
    fun assertSuccess(): FormsDownloadResultPage {
        assertText(com.jed.optima.strings.R.string.all_downloads_succeeded)
        return this
    }

    fun showDetails(): ErrorPage {
        onView(withText(getTranslatedString(com.jed.optima.strings.R.string.show_details))).perform(click())

...
public class ChangesReasonPromptPage extends Page<ChangesReasonPromptPage> {
    ...
    public ChangesReasonPromptPage assertOnPage() {
        ...
        onView(withText(getTranslatedString(com.jed.optima.strings.R.string.reason_for_changes))).check(matches(isDisplayed()));
        return this;
    }

    public ChangesReasonPromptPage enterReason(String reason) {
        onView(withHint(getTranslatedString(com.jed.optima.strings.R.string.reason))).perform(replaceText(reason));
        return this;
    }

    public MainMenuPage clickSave() {
        clickOnString(com.jed.optima.strings.R.string.save);
    ...
    public <D extends Page<D>> D clickSave(D destination) {
        ...
        WaitFor.waitFor((Callable<Void>) () -> {
            assertTextDoesNotExist(com.jed.optima.strings.R.string.saving_form);
            return null;
        });
    ...
    public <D extends Page<D>> D pressClose(D destination) {
        onView(withContentDescription(getTranslatedString(com.jed.optima.strings.R.string.close))).perform(click());
        return destination.assertOnPage();
    }

 ...
 * @author paulburke
 */
class MediaUtils(private val intentLauncher: IntentLauncher, private val contentUriProvider: com.jed.optima.android.utilities.ContentUriProvider) {
    fun deleteMediaFile(imageFile: String) {
        com.jed.optima.android.utilities.FileUtils.deleteAndReport(File(imageFile))
    }

    fun openFile(context: Context, file: File, expectedMimeType: String?) {
        if (!file.exists()) {
            val errorMsg: String = context.getString(com.jed.optima.strings.R.string.file_missing, file)
        ...
        intentLauncher.launch(context, intent) {
            val message = context.getString(
                ...
                context.getString(com.jed.optima.strings.R.string.open_file)
            )
    ...
    private fun getMimeType(file: File, expectedMimeType: String?) =
        if (expectedMimeType == null || expectedMimeType.isEmpty()) {
            com.jed.optima.android.utilities.FileUtils.getMimeType(file)
        } else {
            expectedMimeType
        }
    ...
    fun isVideoFile(file: File): Boolean {
        return com.jed.optima.android.utilities.FileUtils.getMimeType(file).startsWith("video")
    }

    fun isImageFile(file: File): Boolean {
        return com.jed.optima.android.utilities.FileUtils.getMimeType(file).startsWith("image")
    }

    fun isAudioFile(file: File): Boolean {
        return com.jed.optima.android.utilities.FileUtils.getMimeType(file).startsWith("audio")
    }
}

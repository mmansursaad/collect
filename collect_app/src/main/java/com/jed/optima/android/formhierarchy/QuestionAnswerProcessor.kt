...
object QuestionAnswerProcessor {
    ...
    fun getQuestionAnswer(fep: FormEntryPrompt, context: Context, formController: FormController): String {
        ...
        if (data is DateTimeData) {
            return com.jed.optima.android.widgets.utilities.DateTimeWidgetUtils.getDateTimeLabel(
                data.getValue() as Date,
                com.jed.optima.android.widgets.utilities.DateTimeWidgetUtils.getDatePickerDetails(appearance),
                true,
                context
            )
        }

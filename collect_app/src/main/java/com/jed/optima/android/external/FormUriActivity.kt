...
import com.jed.optima.android.instancemanagement.InstanceDeleter
import com.jed.optima.android.instancemanagement.canBeEdited
import com.jed.optima.android.projects.ProjectsDataService
import com.jed.optima.android.savepoints.SavepointUseCases
...
class FormUriActivity : LocalizedActivity() {
    ...
    private fun displaySavePointRecoveryDialog(savepoint: Savepoint) {
        MaterialAlertDialogBuilder(this)
            ...
            .setPositiveButton(string.recover) { _, _ ->
                ...
                if (uriMimeType == com.jed.optima.android.external.FormsContract.CONTENT_ITEM_TYPE) {
                    startForm(com.jed.optima.android.external.FormsContract.getUri(projectsDataService.requireCurrentProject().uuid, savepoint.formDbId))
                } else {
    ...
    private fun startForm(uri: Uri) {
        ...
        openForm.launch(
            Intent(this, com.jed.optima.android.activities.FormFillingActivity::class.java).apply {
...
private class FormUriViewModel(
    ...
    private fun assertValidUri(): String? {
        val isUriValid = uri?.let {
            ...
            if (uriMimeType == null) {
                ...
                uriMimeType == com.jed.optima.android.external.FormsContract.CONTENT_ITEM_TYPE || uriMimeType == com.jed.optima.android.external.InstancesContract.CONTENT_ITEM_TYPE
            }
        } ?: false
    ...
    private fun assertFormNotEncrypted(): String? {
        ...
        return if (uriMimeType == com.jed.optima.android.external.InstancesContract.CONTENT_ITEM_TYPE) {
            val instance = instancesRepositoryProvider.create().get(ContentUriHelper.getIdFromUri(uri))
    ...
    private fun assertDoesNotUseEntitiesOrFormsUpdateNotInProgress(): String? {
        ...
        val form = if (uriMimeType == com.jed.optima.android.external.FormsContract.CONTENT_ITEM_TYPE) {
    ...
    private fun canFormBeEdited(): Boolean {
        ...
        val formEditingEnabled = if (uriMimeType == com.jed.optima.android.external.InstancesContract.CONTENT_ITEM_TYPE) {
            val instance = instancesRepositoryProvider.create().get(ContentUriHelper.getIdFromUri(uri))
...
private sealed class FormInspectionResult {
    ...
    data class Savepoint(val savepoint: com.jed.optima.forms.savepoints.Savepoint) : FormInspectionResult()

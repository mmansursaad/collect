...
import com.jed.optima.android.formmanagement.metadata.FormMetadata
import com.jed.optima.android.formmanagement.metadata.FormMetadataParser
...
object LocalFormUseCases {
    ...
    fun synchronizeWithDisk(formsRepository: com.jed.optima.forms.FormsRepository, formsDir: String?): String {
        var statusMessage = ""
        ...
        return try {
            ...
            if (formDir.exists() && formDir.isDirectory) {
                ...
                for (entry in uriToUpdate) {
                    ...
                    // Probably someone overwrite the file on the sdcard
                    // So re-parse it and update it's information
                    var form: com.jed.optima.forms.Form
                    form = try {
                    ...
                    formsRepository.save(
                        com.jed.optima.forms.Form.Builder(form)
                            .dbId(entry.id)
                            .build()
                    )
                }
            ...
            if (errors.length != 0) {
                ...
                Timber.d(
                    com.jed.optima.android.application.Collect.getInstance()
                        .getLocalizedString(com.jed.optima.strings.R.string.finished_disk_scan)
                )
            }
            statusMessage
        } finally {
    ...
    private fun parseForm(formDefFile: File?): com.jed.optima.forms.Form {
        // Probably someone overwrite the file on the sdcard
        // So re-parse it and update it's information
        val builder = com.jed.optima.forms.Form.Builder()
        ...
        if (title != null) {
            ...
            throw IllegalArgumentException(
                com.jed.optima.android.application.Collect.getInstance()
                    .getLocalizedString(
                        com.jed.optima.strings.R.string.xform_parse_error,
                        formDefFile!!.name,
                        "title"
                    )
            )
        ...
        if (submission != null) {
            if (Validator.isUrlValid(submission)) {
                ...
                throw IllegalArgumentException(
                    com.jed.optima.android.application.Collect.getInstance().getLocalizedString(
                        com.jed.optima.strings.R.string.xform_parse_error,
                        formDefFile!!.name,
                        "submission url"
                    )
        ...
        builder.formMediaPath(
            com.jed.optima.android.utilities.FileUtils.constructMediaPath(
                formDefFile.absolutePath
            )
        )

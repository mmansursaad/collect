...
import com.jed.optima.shared.strings.StringUtils.removeEnd
...
object GeoWidgetUtils {
    ...
    private fun getCardinalDirection(context: Context, coordinate: Double, type: String): String {
        return if (type.equals("lon", ignoreCase = true)) {
            if (coordinate < 0) {
                context.getString(com.jed.optima.strings.R.string.west)
            } else {
                context.getString(com.jed.optima.strings.R.string.east)
            }
        } else if (coordinate < 0) {
            context.getString(com.jed.optima.strings.R.string.south)
        } else {
            context.getString(com.jed.optima.strings.R.string.north)
        }
    }
}

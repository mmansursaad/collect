...
import com.jed.optima.androidshared.ui.DialogFragmentUtils.showIfNotShowing
...
class FormEntryMenuProvider(
    ...
    private val answersProvider: com.jed.optima.android.formentry.questions.AnswersProvider,
    private val formEntryViewModel: com.jed.optima.android.formentry.FormEntryViewModel,
    private val audioRecorder: AudioRecorder,
    private val backgroundLocationViewModel: com.jed.optima.android.formentry.backgroundlocation.BackgroundLocationViewModel,
    private val backgroundAudioViewModel: com.jed.optima.android.formentry.BackgroundAudioViewModel,
    ...
    override fun onPrepareMenu(menu: Menu) {
        ...
        if (formController != null &&
            ...
            com.jed.optima.androidshared.system.PlayServicesChecker().isGooglePlayServicesAvailable(activity)
        ) {
            ...
            menu.findItem(R.id.track_location).icon = if (isBackgroundLocationEnabled) {
                ContextCompat.getDrawable(activity, com.jed.optima.icons.R.drawable.ic_baseline_location_on_24)
            } else {
                ContextCompat.getDrawable(activity, com.jed.optima.icons.R.drawable.ic_baseline_location_off_24)
            }
            menu.findItem(R.id.track_location).title = if (isBackgroundLocationEnabled) {
                activity.getLocalizedString(com.jed.optima.strings.R.string.track_location_on)
            } else {
        ...
        menu.findItem(R.id.menu_record_audio).icon = if (isRecordingAudioEnabled) {
            ContextCompat.getDrawable(activity, com.jed.optima.icons.R.drawable.ic_baseline_mic_24)
        } else {
            ContextCompat.getDrawable(activity, com.jed.optima.icons.R.drawable.ic_baseline_mic_off_24)
        }
        menu.findItem(R.id.menu_record_audio).title = if (isRecordingAudioEnabled) {
            activity.getLocalizedString(com.jed.optima.strings.R.string.record_audio_on)
        } else {
            activity.getLocalizedString(com.jed.optima.strings.R.string.record_audio_off)
        }
    }
        ...
        return when (item.itemId) {
            R.id.menu_add_repeat -> {
                if (audioRecorder.isRecording() && !backgroundAudioViewModel.isBackgroundRecording) {
                    showIfNotShowing(com.jed.optima.android.formentry.RecordingWarningDialogFragment::class.java, activity.supportFragmentManager)
                } else {
            ...
            R.id.menu_preferences -> {
                if (audioRecorder.isRecording()) {
                    ...
                    activity.startActivityForResult(intent, com.jed.optima.android.utilities.ApplicationConstants.RequestCodes.CHANGE_SETTINGS)
                }
                true
            }
            R.id.track_location -> {
            ...
            R.id.menu_goto -> {
                if (audioRecorder.isRecording() && !backgroundAudioViewModel.isBackgroundRecording) {
                    ...
                    activity.startActivityForResult(i, com.jed.optima.android.utilities.ApplicationConstants.RequestCodes.HIERARCHY_ACTIVITY)
                }
                true
            }
            R.id.menu_record_audio -> {
                val enabled = item.title == activity.getLocalizedString(com.jed.optima.strings.R.string.record_audio_off)
                if (enabled) {
                    MaterialAlertDialogBuilder(activity)
                        .setMessage(com.jed.optima.strings.R.string.background_audio_recording_enabled_explanation)
                        .setCancelable(false)
                    ...
                    MaterialAlertDialogBuilder(activity)
                        .setMessage(com.jed.optima.strings.R.string.stop_recording_confirmation)
                        .setPositiveButton(com.jed.optima.strings.R.string.disable_recording) { _: DialogInterface?, _: Int -> backgroundAudioViewModel.setBackgroundRecordingEnabled(false) }
                        .setNegativeButton(com.jed.optima.strings.R.string.cancel, null)
                        .create()
                        .show()
                }
                true

...
class ImageCompressionController(private val imageCompressor: ImageCompressor) {
    fun execute(
        imagePath: String,
        questionWidget: com.jed.optima.android.widgets.QuestionWidget,
        context: Context,
    ...
    private fun getMaxPixelsFromFormIfDefined(questionWidget: com.jed.optima.android.widgets.QuestionWidget): Int? {
        for (bindAttribute in questionWidget.formEntryPrompt.bindAttributes) {
            if ("max-pixels" == bindAttribute.name && com.jed.optima.android.utilities.ApplicationConstants.Namespaces.XML_OPENROSA_NAMESPACE == bindAttribute.namespace) {
                try {

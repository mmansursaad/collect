...
import com.jed.optima.android.databinding.FormEntryEndBinding
...
class FormEndView(
    ...
    init {
        binding.description.text = context.getString(com.jed.optima.strings.R.string.save_enter_data_description /*formTitle*/)
        ...
        if (shouldFormBeSentAutomatically) {
            binding.finalize.text = context.getString(com.jed.optima.strings.R.string.send)
        }
            ...
            if (shouldFormBeSentAutomatically) {
                if (isFormEditableAfterFinalization) {
                    setWarning(
                        ...
                        title = com.jed.optima.strings.R.string.form_editing_enabled_after_sending,
                        hint = com.jed.optima.strings.R.string.form_editing_enabled_after_sending_hint
                    )
                } else {
                    setWarning(
                        icon = R.drawable.ic_edit_off_24,
                        title = com.jed.optima.strings.R.string.form_editing_disabled_after_sending,
                        hint = com.jed.optima.strings.R.string.form_editing_disabled_hint
                    )
                }
            } else {
                if (isFormEditableAfterFinalization) {
                    setWarning(
                        icon = R.drawable.ic_edit_24,
                        title = com.jed.optima.strings.R.string.form_editing_enabled_after_finalizing,
                        hint = com.jed.optima.strings.R.string.form_editing_enabled_after_finalizing_hint
                    )
                } else {
                    setWarning(
                        icon = R.drawable.ic_edit_off_24,
                        title = com.jed.optima.strings.R.string.form_editing_disabled_after_finalizing,
                        hint = com.jed.optima.strings.R.string.form_editing_disabled_hint
                    )
                }
            }
        ...
        return SpannableStringBuilder().inSpans(
            span = object : ClickableSpan() {
                override fun onClick(view: View) {
                    val intent = Intent(context, com.jed.optima.webpage.WebViewActivity::class.java)
                ...
                inSpans(
                    ...
                    builderAction = {
                        color(ContextUtils.getThemeAttributeValue(context, com.google.android.material.R.attr.colorAccent)) {
                            underline {
                                append(context.getLocalizedString(com.jed.optima.strings.R.string.form_edits_warning_learn_more))
                            }
                        }
                    }
                )

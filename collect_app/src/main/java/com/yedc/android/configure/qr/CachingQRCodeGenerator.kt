...
class CachingQRCodeGenerator(private val qrCodeCreator: QRCodeCreator) : QRCodeGenerator {
    ...
    override fun generateQRCode(
        ...
        val writeDir = File(StoragePathProvider().getOdkDirPath(_root_ide_package_.com.yedc.android.storage.StorageSubdirectory.SETTINGS))
        if (!writeDir.exists()) {
            if (!writeDir.mkdirs()) {
        ...
        if (mdCacheFile.exists()) {
            val cachedMessageDigest = _root_ide_package_.com.yedc.android.utilities.FileUtils.read(mdCacheFile)
        ...
        if (shouldWriteToDisk) {
            ...
            _root_ide_package_.com.yedc.android.utilities.FileUtils.write(mdCacheFile, messageDigest)
    ...
    private val qRCodeFilepath: String
        get() = StoragePathProvider().getOdkDirPath(_root_ide_package_.com.yedc.android.storage.StorageSubdirectory.SETTINGS) + File.separator + "com.yedc-settings.png"
    private val md5CachePath: String
        get() = StoragePathProvider().getOdkDirPath(_root_ide_package_.com.yedc.android.storage.StorageSubdirectory.SETTINGS) + File.separator + SETTINGS_MD5_FILE

    companion object {
        private const val SETTINGS_MD5_FILE = ".com.yedc-settings-hash"
    }
}

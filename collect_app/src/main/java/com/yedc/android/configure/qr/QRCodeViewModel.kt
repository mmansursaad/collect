        ...
        scheduler.immediate(
            ...
            if (serverPasswordSet || adminPasswordSet) {
                if (serverPasswordSet && includedKeys.contains(ProjectKeys.KEY_PASSWORD) && adminPasswordSet && includedKeys.contains(
                    ...
                    _warning.setValue(com.yedc.strings.R.string.qrcode_with_both_passwords)
                } else if (serverPasswordSet && includedKeys.contains(ProjectKeys.KEY_PASSWORD)) {
                    _warning.setValue(com.yedc.strings.R.string.qrcode_with_server_password)
                } else if (adminPasswordSet && includedKeys.contains(ProtectedProjectKeys.KEY_ADMIN_PW)) {
                    _warning.setValue(com.yedc.strings.R.string.qrcode_with_admin_password)
                } else {
                    _warning.setValue(com.yedc.strings.R.string.qrcode_without_passwords)
                }
            } else {

...
class ImageCompressionController(private val imageCompressor: ImageCompressor) {
    fun execute(
        imagePath: String,
        questionWidget: _root_ide_package_.com.yedc.android.widgets.QuestionWidget,
        context: Context,
    ...
    private fun getMaxPixelsFromFormIfDefined(questionWidget: _root_ide_package_.com.yedc.android.widgets.QuestionWidget): Int? {
        for (bindAttribute in questionWidget.formEntryPrompt.bindAttributes) {
            if ("max-pixels" == bindAttribute.name && _root_ide_package_.com.yedc.android.utilities.ApplicationConstants.Namespaces.XML_OPENROSA_NAMESPACE == bindAttribute.namespace) {
                try {

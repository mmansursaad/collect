 ...
 * @author paulburke
 */
class MediaUtils(private val intentLauncher: IntentLauncher, private val contentUriProvider: _root_ide_package_.com.yedc.android.utilities.ContentUriProvider) {
    fun deleteMediaFile(imageFile: String) {
        _root_ide_package_.com.yedc.android.utilities.FileUtils.deleteAndReport(File(imageFile))
    }

    fun openFile(context: Context, file: File, expectedMimeType: String?) {
        if (!file.exists()) {
            val errorMsg: String = context.getString(com.yedc.strings.R.string.file_missing, file)
        ...
        intentLauncher.launch(context, intent) {
            val message = context.getString(
                ...
                context.getString(com.yedc.strings.R.string.open_file)
            )
    ...
    private fun getMimeType(file: File, expectedMimeType: String?) =
        if (expectedMimeType == null || expectedMimeType.isEmpty()) {
            _root_ide_package_.com.yedc.android.utilities.FileUtils.getMimeType(file)
        } else {
            expectedMimeType
        }
    ...
    fun isVideoFile(file: File): Boolean {
        return _root_ide_package_.com.yedc.android.utilities.FileUtils.getMimeType(file).startsWith("video")
    }

    fun isImageFile(file: File): Boolean {
        return _root_ide_package_.com.yedc.android.utilities.FileUtils.getMimeType(file).startsWith("image")
    }

    fun isAudioFile(file: File): Boolean {
        return _root_ide_package_.com.yedc.android.utilities.FileUtils.getMimeType(file).startsWith("audio")
    }
}

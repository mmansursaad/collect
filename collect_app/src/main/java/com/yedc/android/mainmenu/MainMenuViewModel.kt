...
import com.yedc.android.instancemanagement.autosend.shouldFormBeSentAutomatically
import com.yedc.android.instancemanagement.canBeEdited
import com.yedc.android.instancemanagement.isDraft
...
class MainMenuViewModel(
    ...
    private fun isMatchExactlyEnabled(): Boolean {
        return settingsProvider.getUnprotectedSettings()
            .getFormUpdateMode(application) == com.yedc.settings.enums.FormUpdateMode.MATCH_EXACTLY
    }
    ...
    fun refreshInstances() {
        scheduler.immediate<Any?>({
            _root_ide_package_.com.yedc.android.instancemanagement.InstanceDiskSynchronizer(
                settingsProvider
            ).doInBackground()
    ...
    private fun getFormSavedSnackbarDetails(uri: Uri): Pair<Int, Int?>? {
        ...
        return if (instance != null) {
            val message = if (instance.isDraft()) {
                com.yedc.strings.R.string.form_saved_as_draft
            } else if (instance.status == com.yedc.forms.instances.Instance.STATUS_COMPLETE || instance.status == com.yedc.forms.instances.Instance.STATUS_SUBMISSION_FAILED) {
                ...
                if (form.shouldFormBeSentAutomatically(autoSendSettingsProvider.isAutoSendEnabledInSettings())) {
                    com.yedc.strings.R.string.form_sending
                } else {
                    com.yedc.strings.R.string.form_saved
                }
            } else {
                return null
            }
            ...
            val action = if (instance.canBeEdited(settingsProvider)) {
                com.yedc.strings.R.string.edit_form
            } else {
                if (instance.isDraft() || instance.canEditWhenComplete()) {
                    com.yedc.strings.R.string.view_form
                } else {
                    null
                }
            }

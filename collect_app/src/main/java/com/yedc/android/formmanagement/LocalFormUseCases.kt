...
import com.yedc.android.formmanagement.metadata.FormMetadata
...
object LocalFormUseCases {
    ...
    fun synchronizeWithDisk(formsRepository: com.yedc.forms.FormsRepository, formsDir: String?): String {
        var statusMessage = ""
            ...
            if (formDir.exists() && formDir.isDirectory) {
                ...
                for (entry in uriToUpdate) {
                    ...
                    formsRepository.save(
                        com.yedc.forms.Form.Builder(form)
                            .dbId(entry.id)
                            .build()
                    )
                }
            ...
            if (errors.length != 0) {
                ...
                Timber.d(
                    _root_ide_package_.com.yedc.android.application.Collect.getInstance()
                        .getLocalizedString(com.yedc.strings.R.string.finished_disk_scan)
                )
            }
            statusMessage
        } finally {
    ...
    private fun parseForm(formDefFile: File?): com.yedc.forms.Form {
        // Probably someone overwrite the file on the sdcard
        // So re-parse it and update it's information
        val builder = com.yedc.forms.Form.Builder()
        ...
        if (title != null) {
            ...
            throw IllegalArgumentException(
                _root_ide_package_.com.yedc.android.application.Collect.getInstance()
                    .getLocalizedString(
                        com.yedc.strings.R.string.xform_parse_error,
                        formDefFile!!.name,
                        "title"
                    )
            )
        ...
        if (submission != null) {
            if (Validator.isUrlValid(submission)) {
                ...
                throw IllegalArgumentException(
                    _root_ide_package_.com.yedc.android.application.Collect.getInstance().getLocalizedString(
                        com.yedc.strings.R.string.xform_parse_error,
                        formDefFile!!.name,
                        "submission url"
                    )
        ...
        builder.formMediaPath(
            _root_ide_package_.com.yedc.android.utilities.FileUtils.constructMediaPath(
                formDefFile.absolutePath
            )
        )

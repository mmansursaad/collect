...
class MultiSelectViewModel<T>(
    ...
    private val isAllSelected = _root_ide_package_.com.jed.optima.androidshared.livedata.LiveDataUtils.zip(data, selected).map { (data, selected) ->
...
fun updateSelectAll(button: Button, itemCount: Int, selectedCount: Int): Boolean {
    ...
    if (allSelected) {
        button.setText(com.jed.optima.strings.R.string.clear_all)
    } else {
        button.setText(com.jed.optima.strings.R.string.select_all)
    }

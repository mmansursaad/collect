...
import com.yedc.shared.files.FileExt.listFilesRecursively
import com.yedc.shared.files.FileExt.sanitizedCanonicalPath
import com.yedc.shared.files.FileExt.saveToFile
import java.io.File
...
class FileExtTest {
    ...
    fun `sanitizedCanonicalPath returns sanitized canonicalPath if androidData path segment is incorrect`() {
        val file = mock<File>().apply {
            whenever(canonicalPath).thenReturn("/storage/emulated/0/android/Data/com.yedc.android/files/projects/DEMO/blah")
        }

        assertThat(file.sanitizedCanonicalPath(), equalTo("/storage/emulated/0/Android/data/com.yedc.android/files/projects/DEMO/blah"))
    }

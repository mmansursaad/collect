...
object TempFiles {
    ...
    private fun getTempDir(): File {
        val tmpDir = File(System.getProperty("java.io.tmpdir", "."), "com.yedc.shared.TempFiles")
        if (!tmpDir.exists()) {
            tmpDir.mkdir()
        }
    ...
    private fun getRandomName(parent: File): String {
        ...
        var candiate = _root_ide_package_.com.yedc.shared.strings.RandomString.randomString(16)
        while (existing!!.any { it.name.contains(candiate) }) {
            candiate = _root_ide_package_.com.yedc.shared.strings.RandomString.randomString(16)
        }

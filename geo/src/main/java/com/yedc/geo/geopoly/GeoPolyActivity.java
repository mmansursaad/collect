...
package com.yedc.geo.geopoly;

import static com.yedc.geo.Constants.EXTRA_READ_ONLY;
import static com.yedc.geo.GeoActivityUtils.requireLocationPermissions;
...
import com.yedc.geo.Constants;
import com.yedc.geo.GeoDependencyComponentProvider;
import com.yedc.geo.GeoUtils;
import com.yedc.geo.R;
import com.yedc.geo.geopoint.AccuracyStatusView;
import com.yedc.geo.geopoint.LocationAccuracy;
import com.yedc.location.Location;
import com.yedc.location.tracker.LocationTracker;
import com.yedc.maps.LineDescription;
import com.yedc.maps.MapConsts;
import com.yedc.maps.MapFragment;
import com.yedc.maps.MapFragmentFactory;
import com.yedc.maps.MapPoint;
import com.yedc.maps.layers.OfflineMapLayersPickerBottomSheetDialogFragment;
...
public class GeoPolyActivity extends LocalizedActivity implements GeoPolySettingsDialogFragment.SettingsDialogCallback {
    ...
    @Override public void onCreate(Bundle savedInstanceState) {
        ...
        setTitle(getString(outputMode == OutputMode.GEOTRACE ?
            com.yedc.strings.R.string.geotrace_title : com.yedc.strings.R.string.geoshape_title));
    ...
    private void saveAsPolyline() {
        if (map.getPolyLinePoints(featureId).size() > 1) {
            ...
            ToastUtils.showShortToastInMiddle(this, getString(com.yedc.strings.R.string.polyline_validator));
        }
    }
    ...
    private void saveAsPolygon() {
        if (map.getPolyLinePoints(featureId).size() > 2) {
            ...
            ToastUtils.showShortToastInMiddle(this, getString(com.yedc.strings.R.string.polygon_validator));
        }
    }
    ...
    private void updateUi() {
        ...
        collectionStatus.setText(
            !inputActive ? getString(com.yedc.strings.R.string.collection_status_paused, numPoints)
                : !recordingEnabled ? getString(com.yedc.strings.R.string.collection_status_placement, numPoints)
                : !recordingAutomatic ? getString(com.yedc.strings.R.string.collection_status_manual, numPoints)
                : !usingThreshold ? (
                    minutes > 0 ?
                        getString(com.yedc.strings.R.string.collection_status_auto_minutes, numPoints, minutes) :
                        getString(com.yedc.strings.R.string.collection_status_auto_seconds, numPoints, seconds)
                )
                : (
                    minutes > 0 ?
                        getString(com.yedc.strings.R.string.collection_status_auto_minutes_accuracy, numPoints, minutes, meters) :
                        getString(com.yedc.strings.R.string.collection_status_auto_seconds_accuracy, numPoints, seconds, meters)
                )
        );
    }
    ...
    private void showClearDialog() {
        if (!map.getPolyLinePoints(featureId).isEmpty()) {
            new MaterialAlertDialogBuilder(this)
                .setMessage(com.yedc.strings.R.string.geo_clear_warning)
                .setPositiveButton(com.yedc.strings.R.string.clear, (dialog, id) -> clear())
    ...
    private void showBackDialog() {
        new MaterialAlertDialogBuilder(this)
            .setMessage(getString(com.yedc.strings.R.string.geo_exit_warning))
            .setPositiveButton(com.yedc.strings.R.string.discard, (dialog, id) -> finish())
            .setNegativeButton(com.yedc.strings.R.string.cancel, null)
            .show();

...
package com.jed.optima.geo.geopoly;

import static com.jed.optima.geo.Constants.EXTRA_READ_ONLY;
import static com.jed.optima.geo.GeoActivityUtils.requireLocationPermissions;
...
import com.jed.optima.geo.Constants;
import com.jed.optima.geo.GeoDependencyComponentProvider;
import com.jed.optima.geo.GeoUtils;
import com.jed.optima.geo.R;
import com.jed.optima.geo.geopoint.AccuracyStatusView;
import com.jed.optima.geo.geopoint.LocationAccuracy;
...
import com.jed.optima.maps.MapConsts;
...
public class GeoPolyActivity extends LocalizedActivity implements GeoPolySettingsDialogFragment.SettingsDialogCallback {
    ...
    @Override public void onCreate(Bundle savedInstanceState) {
        ...
        setTitle(getString(outputMode == OutputMode.GEOTRACE ?
            com.jed.optima.strings.R.string.geotrace_title : com.jed.optima.strings.R.string.geoshape_title));
    ...
    private void saveAsPolyline() {
        if (map.getPolyLinePoints(featureId).size() > 1) {
            ...
            ToastUtils.showShortToastInMiddle(this, getString(com.jed.optima.strings.R.string.polyline_validator));
        }
    }
    ...
    private void saveAsPolygon() {
        if (map.getPolyLinePoints(featureId).size() > 2) {
            ...
            ToastUtils.showShortToastInMiddle(this, getString(com.jed.optima.strings.R.string.polygon_validator));
        }
    }
    ...
    private void updateUi() {
        ...
        collectionStatus.setText(
            !inputActive ? getString(com.jed.optima.strings.R.string.collection_status_paused, numPoints)
                : !recordingEnabled ? getString(com.jed.optima.strings.R.string.collection_status_placement, numPoints)
                : !recordingAutomatic ? getString(com.jed.optima.strings.R.string.collection_status_manual, numPoints)
                : !usingThreshold ? (
                    minutes > 0 ?
                        getString(com.jed.optima.strings.R.string.collection_status_auto_minutes, numPoints, minutes) :
                        getString(com.jed.optima.strings.R.string.collection_status_auto_seconds, numPoints, seconds)
                )
                : (
                    minutes > 0 ?
                        getString(com.jed.optima.strings.R.string.collection_status_auto_minutes_accuracy, numPoints, minutes, meters) :
                        getString(com.jed.optima.strings.R.string.collection_status_auto_seconds_accuracy, numPoints, seconds, meters)
                )
        );
    }
    ...
    private void showClearDialog() {
        if (!map.getPolyLinePoints(featureId).isEmpty()) {
            new MaterialAlertDialogBuilder(this)
                .setMessage(com.jed.optima.strings.R.string.geo_clear_warning)
                .setPositiveButton(com.jed.optima.strings.R.string.clear, (dialog, id) -> clear())
    ...
    private void showBackDialog() {
        new MaterialAlertDialogBuilder(this)
            .setMessage(getString(com.jed.optima.strings.R.string.geo_exit_warning))
            .setPositiveButton(com.jed.optima.strings.R.string.discard, (dialog, id) -> finish())
            .setNegativeButton(com.jed.optima.strings.R.string.cancel, null)
            .show();

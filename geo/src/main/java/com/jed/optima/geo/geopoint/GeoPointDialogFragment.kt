...
import com.jed.optima.geo.GeoUtils.formatAccuracy
import com.jed.optima.geo.databinding.GeopointDialogBinding
...
class GeoPointDialogFragment : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        binding.threshold.text =
            getString(com.jed.optima.strings.R.string.point_will_be_saved, formatAccuracy(context, accuracyThreshold))

        viewModel.timeElapsed.observe(this) {
            binding.time.text =
                getString(com.jed.optima.strings.R.string.time_elapsed, DateUtils.formatElapsedTime(it / 1000))
        }

        viewModel.satellites.observe(this) {
            binding.satellites.text = getString(com.jed.optima.strings.R.string.satellites, it.toString())
        }

        val dialog = MaterialAlertDialogBuilder(requireContext())
            .setView(binding.root)
            .setPositiveButton(com.jed.optima.strings.R.string.save) { _, _ -> viewModel.forceLocation() }
            .setNegativeButton(com.jed.optima.strings.R.string.cancel) { _, _ -> listener?.onCancel() }
            .create()
